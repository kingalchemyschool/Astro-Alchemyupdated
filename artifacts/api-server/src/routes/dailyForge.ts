...
function generateReport(req: DailyForgeRequest): ForgeReport {
  ...
  // Each aspect type maps to its alchemical process.
  // No planets named, no aspect labels, no supporting-aspect list.
  // Prose reads as a teaching, not a data summary.
  const whatIsBeingRefinedByAspect: Record<AspectType, string[]> = {
    conjunction: [
      `There is a point in alchemical work when two substances stop existing separately — when they fuse into a single material denser and more capable than either alone. Today's refinement arrives at that moment in the area of ${nHouseMeaning.full}. The capacity taking shape is ${refinement}, and what is being asked is not exploration across multiple directions but the compression of everything available into one concentrated form. Coagulation does not happen through dispersion. It happens through commitment — through the act of bringing energy into a single point and holding it there long enough for something permanent to crystallize. What is being solidified is not a new idea about ${refinement2}, but a more reliable expression of it: one that can be carried forward rather than reconstructed each time it is needed.`,
      `In the alchemical process of coagulation, dissolved material crystallizes into its final form. What has been fluid must now become fixed. Today's refinement moves through that same transition in the area of ${nHouseMeaning.full}. The developing capacity is ${refinement}, and the work is to stop distributing it across competing intentions and bring it into unified, deliberate expression. What gets concentrated tends to become more potent. What gets dispersed tends to remain unfinished. The alchemical lesson today is the older and harder one: commit to the singular form of ${refinement2} this moment is asking for, and let that commitment do what distributed effort cannot.`,
      `Fusion is not the same as mixture. In mixture, substances remain distinct; in fusion, they become something new. The refinement available today in the area of ${nHouseMeaning.full} belongs to the stage of fusion — where ${refinement} is no longer held as potential but pressed into its most concentrated, committed expression. The alchemist's discipline in this condition is to resist the temptation to keep the process open, to keep exploring in theory. At a certain stage the work becomes coagulation or it becomes stagnation. What crystallizes through today's concentrated engagement is ${refinement2} expressed without division — something that can be relied on, not just described.`,
      `Some alchemical processes work through heat. Others work through time. Today's works through concentration — the act of bringing everything available into one place and holding it there long enough for something permanent to form. In the area of ${nHouseMeaning.full}, the capacity being formed is ${refinement}. The material is ready. What it needs now is not addition or revision but compression. The practitioner who commits to one consolidated direction today will produce a version of ${refinement2} that does not need to be rebuilt tomorrow.`,
    ],
    trine: [
      `The alchemical stage of dissolution softens what has become rigid, making the true composition of a substance visible without the interference of resistance. Today carries that quality in the area of ${nHouseMeaning.full}. The refinement available is ${refinement}, and the conditions allow it to move with less friction than usual — not because the work is easier, but because the medium is clear. What the practitioner learns in this state is that effortlessness is not absence of work; it is work in its most efficient form. The lesson today is to let what flows carry something forward, rather than treating the ease as a rest from the direction that is being built.`,
      `Not all transformation happens under pressure. Some of the most important alchemical work occurs when the medium is clear and the material moves freely. Today's refinement is ${refinement} in the area of ${nHouseMeaning.full}, and the conditions are cooperative enough that what usually requires full force can be accomplished through consistent, unhurried motion. The practitioner's discipline here is intention: ensuring that what flows through today's ease carries a deliberate direction rather than dispersing across unrelated channels. Flow that is not directed produces movement. It does not produce gold. What is being shaped is ${refinement2}, made more substantial by the quality of choices made while conditions favor it.`,
      `There is a stage in the work when the path between intention and result clears. The practitioner who has been developing ${refinement} will recognize today as one of those stages in the area of ${nHouseMeaning.full}. The medium is more fluid, the resistance lower, the available movement longer-ranged than usual. What is being asked is not effort but extension: to push the development of ${refinement2} further than the default stopping point, to allow the ease to be used rather than merely experienced. Flow that has no destination becomes drift. The same flow given a direction becomes the fastest path to something real.`,
      `Dissolution is the alchemical stage that makes rigidity temporary. What has set too hard becomes workable again; what has been defended becomes accessible. Today's refinement moves through that condition in the area of ${nHouseMeaning.full}. The capacity being released into more fluid form is ${refinement}, and the practitioner's task is not simply to experience the ease but to build something within it. Natural momentum does not persist indefinitely. What gets constructed within today's dissolving conditions will outlast the conditions themselves. The alchemy asks: what version of ${refinement2} can be made more permanent through this window while it is open?`,
    ],
    sextile: [
      `Fermentation is the alchemical stage where dormant potential becomes active — where the material, given the right conditions, begins transforming from within. Today holds that kind of opening in the area of ${nHouseMeaning.full}. The capacity available is ${refinement}, and the conditions have become favorable in a way that was not present before. Fermentation does not complete itself; it requires a catalyst — a decision, an initiation, an act taken before full certainty arrives. What the alchemy asks is not readiness but movement: to step into the available opening and let the transformative process begin. The window is present. The question is only whether the practitioner walks through it.`,
      `There is a stage in the work when the conditions for transformation become available without yet becoming automatic. The alchemist calls this the cooperative moment — the opening that requires conscious action to enter and closes without ceremony if left unused. Today's refinement is ${refinement} in the area of ${nHouseMeaning.full}. What separates those who develop ${refinement2} from those who observe it is the decision to begin — not the decision to prepare, not the decision to consider, but the decision to initiate. The cooperative moment rewards movement. It does not reward waiting.`,
      `The alchemical notebook describes certain days as days of opening — when the conditions are favorable and the work moves with less opposition than usual. Today holds that quality in the area of ${nHouseMeaning.full}. The refinement available is ${refinement}, and what distinguishes the practitioner who develops it from the one who merely notices the opportunity is the choice to act within the available window. Fermentation begins with the introduction of a catalyst. In today's work, the catalyst is the decision to initiate. The conditions will not wait for perfect preparation — they reward the practitioner who moves while the opening is present.`,
      `Some alchemical transformations require heat and sustained pressure. Others require only a point of initiation — a moment when the practitioner decides to enter conditions that are already prepared. Today's refinement is ${refinement} in the area of ${nHouseMeaning.full}. The conditions are cooperative. What is being asked is not endurance but the specific courage that precedes certainty: starting before the outcome is guaranteed, moving through the available opening rather than studying it from outside. What gets built within this window will outlast the conditions that created it. The alchemical lesson here is not complex — it is simply: begin.`,
    ],
    square: [
      `Calcination is the alchemical process of applying intense, sustained heat to a substance — burning away everything that cannot withstand fire until only the essential remains. Today's refinement moves through that process in the area of ${nHouseMeaning.full}. There is pressure, and the pressure is the mechanism. What is being calcinated is the less durable form of ${refinement2} — the version that depends on favorable conditions, on things proceeding according to plan. What remains after this fire is the version that holds under real demand. The alchemist who avoids the calcination stage never discovers what the material is actually made of.`,
      `There is an alchemical principle that the quality of the refined material is determined by the quality of the fire it passes through. A weak fire produces an incompletely purified substance. A sustained, precise fire produces something that will not fail under later demand. Today's conditions apply that principle to ${refinement} in the area of ${nHouseMeaning.full}. The friction present is not incidental and not punitive — it marks the exact boundary between the current form of this capacity and the more developed version that sustained pressure is building. Staying in contact with that boundary, engaging rather than routing around it, is how today's fire becomes tomorrow's foundation.`,
      `Calcination does not destroy — it clarifies. By burning away the inessential, it leaves the substance in its most condensed and capable form. Today's refinement of ${refinement} in the area of ${nHouseMeaning.full} follows that same logic. The resistance that arises marks the exact point where the current form of ${refinement2} meets the demand that would make it more durable. The practitioner who works through today's friction rather than around it will arrive somewhere the one who redirects cannot reach. The fire is the instruction. Meeting it directly is the entire work.`,
      `The alchemist who avoids the calcination stage never learns what the material is capable of. What looks like obstacle is load-bearing pressure — the force that determines the final quality of what is being formed. Today's refinement of ${refinement} in the area of ${nHouseMeaning.full} belongs to this stage. The friction is highest where the development is most needed. What is being formed through direct engagement is not a temporary adjustment but a more reliable version of ${refinement2} — built to hold under the kind of pressure present today. This is the alchemy: not to wait for easier conditions, but to let the difficulty do the work it is designed to do.`,
    ],
    opposition: [
      `Separation is the alchemical stage in which a mixed substance is divided into its constituent elements — not to destroy either, but to understand what each actually contains. Today's refinement arrives through that kind of contrast in the area of ${nHouseMeaning.full}. The available information is not coming from within the current position — it is visible only from across it. What is being separated and clarified is the part of ${refinement2} that has been operating as a single view, without the corrective information its opposite provides. The work is not to abandon the current position, but to let its counterpart make it more precise. Distillation requires the full range, not just the comfortable portion of it.`,
      `The alchemist who works with only one pole of any substance will always produce an incomplete distillation. Today's refinement belongs to the stage of separation — where two contrasting forces are held simultaneously, each making the other more precisely visible. The capacity developing is ${refinement} in the area of ${nHouseMeaning.full}. What the contrast asks is not resolution but tolerance: the ability to remain in the field between two positions long enough to receive what neither one alone can provide. Contrast at this stage is not adversarial. It is the mechanism of purification — the process that strips ${refinement2} of the assumption that a single vantage point is sufficient.`,
      `Distillation purifies by passing a substance through contrast — vapour rising from liquid, then condensing back in a more concentrated form. Today's refinement carries that structure in the area of ${nHouseMeaning.full}. The development available is ${refinement}, and it arrives not through accumulation of what is already known, but through genuine contact with the perspective that stands most directly across from the current one. The temptation at this stage is to resolve the contrast quickly — to land on one position and end the discomfort. The alchemical instruction is the opposite: stay in the contrast long enough to receive what it is actually showing you. What is being purified is ${refinement2}, and the purification requires the full range.`,
      `In alchemy, the stage of separation is misread by those who treat it as conflict. It is not — it is the process by which mixed elements become distinct enough to understand, and eventually to be consciously recombined with new clarity. Today's refinement of ${refinement} in the area of ${nHouseMeaning.full} moves through that process. The contrast present is not opposition in the adversarial sense but in the alchemical one: two poles made simultaneously visible, each showing what the other alone cannot. What is being purified is the practitioner's capacity to hold complexity without collapsing it — to develop ${refinement2} large enough to include both poles rather than choosing between them.`,
    ],
  };
