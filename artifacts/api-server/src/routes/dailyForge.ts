...
function generateReport(req: DailyForgeRequest): ForgeReport {
  ...
  const houseFocus = `your ${houseLabel} House — ${nHouseMeaning.full}`;
  ...
  const whatIsBeingRefinedByAspect: Record<AspectType, string[]> = {
    conjunction: [
      `There is a point in alchemical work when two substances stop existing separately — when they fuse into a single material denser and more capable than either alone. Today's refinement arrives at that moment in ${houseFocus}. The capacity taking shape is ${refinement}, and what is being asked is not exploration across multiple directions but the compression of everything available into one concentrated form. Coagulation does not happen through dispersion. It happens through commitment — through the act of bringing energy into a single point and holding it there long enough for something permanent to crystallize. What is being solidified is not a new idea about ${refinement2}, but a more reliable expression of it: one that can be carried forward rather than reconstructed each time it is needed.`,
      `In the alchemical process of coagulation, dissolved material crystallizes into its final form. What has been fluid must now become fixed. Today's refinement moves through that same transition in ${houseFocus}. The developing capacity is ${refinement}, and the work is to stop distributing it across competing intentions and bring it into unified, deliberate expression. What gets concentrated tends to become more potent. What gets dispersed tends to remain unfinished. The alchemical lesson today is the older and harder one: commit to the singular form of ${refinement2} this moment is asking for, and let that commitment do what distributed effort cannot.`,
      `Fusion is not the same as mixture. In mixture, substances remain distinct; in fusion, they become something new. The refinement available today in ${houseFocus} belongs to the stage of fusion — where ${refinement} is no longer held as potential but pressed into its most concentrated, committed expression. The alchemist's discipline in this condition is to resist the temptation to keep the process open, to keep exploring in theory. At a certain stage the work becomes coagulation or it becomes stagnation. What crystallizes through today's concentrated engagement is ${refinement2} expressed without division — something that can be relied on, not just described.`,
      `Some alchemical processes work through heat. Others work through time. Today's works through concentration — the act of bringing everything available into one place and holding it there long enough for something permanent to form. In ${houseFocus}, the capacity being formed is ${refinement}. The material is ready. What it needs now is not addition or revision but compression. The practitioner who commits to one consolidated direction today will produce a version of ${refinement2} that does not need to be rebuilt tomorrow.`,
    ],
    trine: [
      `The alchemical stage of dissolution softens what has become rigid, making the true composition of a substance visible without the interference of resistance. Today carries that quality in ${houseFocus}. The refinement available is ${refinement}, and the conditions allow it to move with less friction than usual — not because the work is easier, but because the medium is clear. What the practitioner learns in this state is that effortlessness is not absence of work; it is work in its most efficient form. The interaction between the two influences is one of release: one loosens what has been fixed, while the other shows where that newly available movement can go.`,
      `Not all transformation happens under pressure. Some of the most important alchemical work occurs when the medium is clear and the material moves freely. Today's refinement is ${refinement} in ${houseFocus}, and the conditions are cooperative enough that what usually requires full force can be accomplished through consistent, unhurried motion. One influence opens the passage; the other supplies the material that can move through it. Their relationship is not asking for a dramatic breakthrough, but showing how ${refinement2} changes when it no longer has to fight for room.`,
      `There is a stage in the work when the path between intention and result clears. The practitioner who has been developing ${refinement} will recognize today as one of those stages in ${houseFocus}. The medium is more fluid, the resistance lower, and the established pattern can respond to something new without breaking first. What becomes visible through that interaction is a version of ${refinement2} that can travel further than its usual limits.`,
      `Dissolution is the alchemical stage that makes rigidity temporary. What has set too hard becomes workable again; what has been defended becomes accessible. Today's refinement moves through that condition in ${houseFocus}. The capacity being released into more fluid form is ${refinement}. One current is softening the material while the other reveals its underlying composition; the lesson is to notice what becomes possible when those two movements cooperate.`,
    ],
    sextile: [
      `Fermentation is the alchemical stage where dormant potential becomes active — where the material, given the right conditions, begins transforming from within. Today holds that kind of opening in ${houseFocus}. The capacity available is ${refinement}, and the conditions have become favorable in a way that was not present before. Fermentation does not complete itself; it requires a catalyst — a decision, an initiation, an act taken before full certainty arrives. One influence creates the opening; the other shows what is ready to develop through it. The alchemical question is whether those two conditions meet in an actual beginning.`,
      `There is a stage in the work when the conditions for transformation become available without yet becoming automatic. The alchemist calls this the cooperative moment — the opening that requires conscious action to enter. Today's refinement is ${refinement} in ${houseFocus}. One force has made room; the other has something that can grow in that room. Their interaction does not promise an outcome, but it does make the first movement meaningful. What separates potential from development is the moment the two are brought into contact.`,
      `The alchemical notebook describes certain days as days of opening — when the conditions are favorable and the work moves with less opposition than usual. Today holds that quality in ${houseFocus}. The refinement available is ${refinement}. Fermentation begins when the catalyst meets prepared material: the opening alone is not enough, and the latent capacity alone cannot begin the process. Together they create a threshold at which a small act can change the whole condition of the work.`,
      `Some alchemical transformations require heat and sustained pressure. Others require only a point of initiation — a moment when the practitioner enters conditions that are already prepared. Today's refinement is ${refinement} in ${houseFocus}. The cooperative force is not replacing the natal capacity; it is giving that capacity a new way to express itself. The lesson is to recognize the interaction clearly: one makes movement possible, while the other determines what that movement is about.`,
    ],
    square: [
      `Calcination is the alchemical process of applying intense, sustained heat to a substance — burning away everything that cannot withstand fire until only the essential remains. Today's refinement moves through that process in ${houseFocus}. There is pressure, and the pressure is the mechanism. What is being calcinated is the less durable form of ${refinement2} — the version that depends on favorable conditions, on things proceeding according to plan. The immediate demand exposes the difference between the capacity as it has been practiced and the capacity as it must become.`,
      `There is an alchemical principle that the quality of the refined material is determined by the quality of the fire it passes through. Today's conditions apply that principle to ${refinement} in ${houseFocus}. One influence brings the heat; the other reveals where the existing structure cannot yet hold it. The friction is therefore specific: it marks the boundary between the current form of this capacity and the more developed version that sustained pressure is building. Staying with that exact boundary is how the interaction becomes refinement rather than mere difficulty.`,
      `Calcination does not destroy — it clarifies. By burning away the inessential, it leaves the substance in its most condensed and capable form. Today's refinement of ${refinement} in ${houseFocus} follows that same logic. The resistance marks the point where the current form of ${refinement2} meets the demand that would make it more durable. The heat is coming from one side; the material being tested is on the other. Their contact shows what must change.`,
      `The alchemist who avoids the calcination stage never learns what the material is capable of. What looks like obstacle is load-bearing pressure — the force that determines the final quality of what is being formed. Today's refinement of ${refinement} in ${houseFocus} belongs to this stage. The pressure is not asking for a general improvement; it is exposing one precise place where ${refinement2} needs greater strength. The work is to understand that interaction and let the difficulty identify the structure that must be rebuilt.`,
    ],
    opposition: [
      `Separation is the alchemical stage in which a mixed substance is divided into its constituent elements — not to destroy either, but to understand what each actually contains. Today's refinement arrives through that kind of contrast in ${houseFocus}. The available information is not coming from within the current position; it is visible only from across it. One influence exposes the limits of the familiar view, while the other supplies the material that must be seen more clearly. What is being separated is the part of ${refinement2} that has been operating as a single, unquestioned perspective.`,
      `The alchemist who works with only one pole of any substance will always produce an incomplete distillation. Today's refinement belongs to the stage of separation, where two contrasting forces are held simultaneously and each makes the other more precise. The capacity developing is ${refinement} in ${houseFocus}. The contrast is not asking for immediate resolution. It is showing how one side changes the meaning of the other, and why ${refinement2} cannot be understood fully from only the position that feels most familiar.`,
      `Distillation purifies by passing a substance through contrast — vapour rising from liquid, then condensing back in a more concentrated form. Today's refinement carries that structure in ${houseFocus}. The development available is ${refinement}, and it arrives through contact with a perspective that stands across the current one. One pole provides the established material; the other draws out what has been hidden inside it. The result is not a compromise between them, but a clearer substance than either could produce alone.`,
      `In alchemy, the stage of separation is misread by those who treat it as conflict. It is the process by which mixed elements become distinct enough to understand, and eventually to be consciously recombined with new clarity. Today's refinement of ${refinement} in ${houseFocus} moves through that process. The contrast makes two poles visible at once: one shows the pattern already in place, while the other reveals what that pattern leaves out. What is being purified is ${refinement2} — not by choosing one side, but by allowing each side to complete the information of the other.`,
    ],
  };
